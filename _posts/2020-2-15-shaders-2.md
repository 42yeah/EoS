...
The fact is, we don't! We are not going to do the method described above. However, here's what we know. If something has been occluded, that means what occludes it must be closer to the camera. Which in turn means when we shoot those vertices out, what we __expect__ to see is a uniform depth (perhaps with a little bit of bias) - but when the depth changes violently, especially when the depth is far less (which means far closer to the camera), we would know occlusion happened here.
