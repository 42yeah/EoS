...
Now, I've implemented one in Javascript. You could play with it for a little bit, however I am not really sure whether my generator could get the same result with Maxim Gumin's or not. Also I will cap N at 3 (Trust me, you browser won't want to go beyond). The color stripe at the bottom is the palatte. Use it to switch colors. Beware that if white cells appear, that indicates a __contradiction__ was met, and you need to generate again.
...
Cool, eh? The thing I implement almost works, however it has a major drawback: it is __extremely slow__. Regardless, I think it's worth spending 3 days? Or how many? Looking into sources and stuffs. So let's implement it again! I will be using buttload of pseudocodes down below. And hopefully you can understand!
...
As the wave has been initialized, we are about to enter a loop to collapse all those waves, until in each cell, there is only one possible pattern left. If we could reach such circumstance, then a __done__ scenairo happened. What if there is one (or multiple) cells got no possible patterns left? Then we've reached a __contradiction__! But how would we get to those scenarioes? The first thing we do is __observe__.
...
Well, Gumin is using the "lowest entropy" rule, and so are the others, so we are going to do the same!  __"entropy" here means the sum of all possible patterns' frequencies inside a cell.__ By finding out the lowest entropy cell in the potential output (wave), and collapsing it into a definite state, we could make sure that the effect of this collapse is at minimum (so we don't have to propagate that much changes).
...
![The Observation phase](/assets/wfc/observe.jpg)
...
Imagine all flagged cells as an array, so we could get one flagged cell out at a time, like a queue. In fact, the flagged array __is__ a queue. Take one down, propagate around, ...
...
![Propagation](/assets/wfc/propagate.jpg)
...
So that's it! Wave Function Collapsed, the overlapping model. I might be wrong, and if I am, __please__ tell me. Also the version I written is, again, __extremely slow__. So don't use mine! Use the others! I am using the others as well. And there you have it!
...
5. [The Very Simple Tiled Model by Robert Heaton](https://github.com/robert/wavefunction-collapse)
